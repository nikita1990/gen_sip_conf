...
```bash
python3 gen_sip_conf_v1.3.py -n <trunk_name> -d <domain> -u <username> -ps <password> -pn <phone_number> -id <client_id> -prefix <prefix> [-op <outbound_proxy>] [-exp <exception_domain>] [-v]
```
...
| `-id`, `--client_id`            | Идентификатор клиента / Client identifier                          | Да / Yes                | `1001`               |
...
```bash
python3 gen_sip_conf_v1.3.py -n my_trunk -d sip.example.com -u user123 -ps mypassword -pn 1234567890 -id 1001 -prefix 9 -op proxy.example.com -exp specialdomain.com -v
```
